function F=makeRFSfilters(NORIENT)
...
% Fourier transform.
% Reference: A statistical approach to texture classification from single
% images, IJCV.
...
if nargin<1
  ...
  SUP=23;                 % Support of the largest filter (must be odd)
  SCALEX=[1,2,4];         % Sigma_{x} for the oriented filters
  

  NROTINV=2;                % Number of rotation invariant filters
  NBAR=length(SCALEX)*NORIENT;  % Number of Bar filters
  NEDGE=length(SCALEX)*NORIENT; % Number of Edge filters
